...
test("get", (t) => {
    let a = new ArrayEList<number>([1, 2, 3, 4]);
    t.is(a.get(0), 1);
    t.is(a.get(1), 2);
    t.is(a.get(2), 3);
    t.is(a.get(3), 4);
...
test("add_unique", (t) => {
    let a = new ArrayEList<number>([], true);
    t.true(a.add(0));
    t.false(a.add(0));
...
test("addAll", (t) => {
    let a = new ArrayEList<number>([0, 1, 2]);
    let b = new ArrayEList<number>([3, 4, 5]);
    t.true(a.addAll(b));
    t.deepEqual(a.toArray(), [0, 1, 2, 3, 4, 5]);
});

test("addAll_unique", (t) => {
    let a = new ArrayEList<number>([0, 1, 2], true);
    let b = new ArrayEList<number>([1, 2, 3]);
    t.true(a.addAll(b));
...
test("removeAll", (t) => {
    let a = new ArrayEList<number>([0, 1, 2]);
    let b = new ArrayEList<number>([1, 2]);
...
test("iterator", (t) => {
    let a = new ArrayEList<number>([3, 4, 5]);
    let v: number[] = [];
    for (const i of a) {
        v.push(i);
    }
